...
`甲烷/天然气检测 + CO 检测 + 温湿度补偿 + 超声波避障 + 声光报警 + 数据可视化 + 遥控`。
...
-  `Figaro TGS2611`。为了把原型更快做出来，这里推荐用更容易接线和编程的 `Winsen ZC13` 甲烷 UART 模块；答辩时可以写成“`TGS2611/同类 CH4 模块`”，并说明原型阶段选择 UART 成品模块以降低调试难度。
